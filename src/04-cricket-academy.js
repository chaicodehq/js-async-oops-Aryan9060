...
export class Batsman extends Player {
  ...
  getProfile() {
    ...
    return {
      ...
      role: "batsman",
      totalRuns: this.innings.reduce((sum, innings) => sum + innings.runs, 0),
      inningsPlayed: this.innings.length,
    };
  }
}
...
export class Bowler extends Player {
  ...
  bowlSpell(wickets, runsConceded, overs) {
    ...
    return this.spells[this.spells.length - 1];
  }

  getEconomy() {
    if (this.spells.length === 0) return 0;
    const totalRuns = this.spells.reduce(
      (sum, spell) => sum + spell.runsConceded,
      0,
    );
    const totalOvers = this.spells.reduce((sum, spell) => sum + spell.overs, 0);
  ...
  getProfile() {
    return {
      ...
      role: "bowler",
      totalWickets: this.spells.reduce((sum, spell) => sum + spell.wickets, 0),
      spellsBowled: this.spells.length,
    };
  }
}
...
export class AllRounder extends Player {
  ...
  getStrikeRate() {
    ...
    const totalBalls = this.innings.reduce((sum, i) => sum + i.balls, 0);
    return (totalRuns / totalBalls) * 100;
  }

  getEconomy() {
    if (this.spells.length === 0) return 0;
    const totalRuns = this.spells.reduce((sum, s) => sum + s.runsConceded, 0);
    const totalOvers = this.spells.reduce((sum, s) => sum + s.overs, 0);
    return totalRuns / totalOvers;
  }
